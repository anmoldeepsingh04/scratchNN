...
Dataloader::Dataloader(Dataset* dataset, int batch_size, bool shuffle) : _dataset(dataset), _batch_size(batch_size){
...
Dataloader::Iterator::Iterator(Dataloader* dataloader, int index) : _dataloader(dataloader), _index(index){

}
void Dataloader::Iterator::operator++(){
    _index += _dataloader -> _batch_size;
}
